function externalScripts() {
  ...
  const mainStyles = `
    ...
    return {
      ...
      checkSlide: function(elems) {
        let boundsArr = [];
        elems.map((itm,idx) => {
            if(idx == 0 || idx === elems.length - 1) {
                boundsArr.push(this.getBounds(itm));
            } 
        })
        return boundsArr;
      },	
      ...
      doCarouselSecondAct: function () {
        ...
        next.addEventListener("click", (e) => {
          ...
          carousel.scrollTop;
          let ecards = Array.from(content.children);
          let ecinfo = this.checkSlide(ecards);
          
          //last one!
          if(ecinfo[1].right <= carousel.offsetWidth + (300*3)) {
              let ls = ecards[ecards.length -1];
              ls.style.border = '1px solid red';
              console.log('last one within viewport Xcoordinate => ', ecinfo[1].right)
          }
         
          console.log(`
              scrolled left ${carousel.scrollLeft}  
              scroll width ${carousel.scrollWidth} 
              current position ${ecinfo[1].right} y: ${ecinfo[1].y}, width: ${ecinfo[1].width}
        ...
        prev.addEventListener("click", (e) => {
          ...
          //first one!
          if(ecinfo[0].left <= 300) {
              let fs = ecards[0]
              fs.style.border = '1px solid blue';
              console.log('first one within viewport => Xcoordinate', ecinfo[0].left);
          }
           ...
           console.log(`
              ...
              current position ${ecinfo[0].left}, y: ${ecinfo[0].y} width: ${ecinfo[0].width}
